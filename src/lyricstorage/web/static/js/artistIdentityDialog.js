...
// 곡 아티스트 정체성(대표 이름 + 이명)을 수정하는 다이얼로그(#artist-info-dialog)는
// 재생 통계 '곡 아티스트' 탭과 브라우즈 '곡 아티스트' 탭이 화면 하나(같은 DOM)를
// 공유한다. 두 화면이 각자 이벤트 리스너를 붙이면 중복 실행되므로, 이 모듈이
// DOM 바인딩을 한 번만 하고 open()으로 호출자별 상태(현재 정체성, 콜백)만 갈아 끼운다.
export function setupArtistIdentityDialog() {
  ...
  function renderChips() {
    ...
    identity.aliases.forEach((alias) => {
      ...
      removeBtn.addEventListener("click", async () => {
        try {
          identity = await api.removeArtistAlias(identity.id, alias);
          onChange(identity);
          renderChips();
          populateSuggestions();
        } catch (err) {
  ...
  aliasAddBtn.addEventListener("click", async () => {
    ...
    if (!alias) return;
    try {
      identity = await api.addArtistAlias(identity.id, alias);
      onChange(identity);
      aliasInput.value = "";
      renderChips();
      populateSuggestions();
  ...
  saveBtn.addEventListener("click", async () => {
    ...
    try {
      if (newName !== identity.name) {
        identity = await api.renameArtist(identity.id, newName);
        onChange(identity);
      }
      dialog.close();
    } catch (err) {
  ...
  return {
    ...
    open(initialIdentity, { getTracks: tracksFn, onChange: onChangeCb, onClose }) {
